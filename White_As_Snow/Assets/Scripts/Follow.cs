...
public class Follow : MonoBehaviour
{
    ...
    private readonly float MIN_D = 5f;
    private readonly float FOLLOW_D = 4.2f;
    ...
    void FixedUpdate()
    {
        ...
        if (moving)
        {
            ...
            else
            {
                ...
                if(getDist(otherFollower) > FOLLOW_D || )
                {
                    if (dirs[0][0])
    ...
    {
        if (getDist(player) < getDist(otherFollower, player))
            return true;
        else return false;
    }
}